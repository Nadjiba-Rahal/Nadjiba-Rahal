<h1 align="center">Hi, I'm Nadjiba</h1>
<h3 align="center">AI & Data Science Engineer | RAG Systems | Algeria</h3>

<p align="center">
Focused on building intelligent systems using Artificial Intelligence, Data Science, and Machine Learning.
</p>

---

## About Me
- Working on Personality Analysis using RAG (Retrieval-Augmented Generation)
- Interested in Applied AI, NLP, and Large Language Models
- Learning German and exploring international research opportunities
- Open to AI internships, research, and collaborations
- Goal: Become an AI Engineer specializing in intelligent systems

---

## Tech Stack

### Programming
Python • JavaScript • C

### Artificial Intelligence / Data Science
Pandas • NumPy • Scikit-learn • PyTorch • TensorFlow • NLP • RAG systems

### Databases
MySQL • PostgreSQL • MongoDB

### Tools
Git • Linux • Jupyter Notebook • VS Code

---

## Featured Projects

### Personality Analysis AI (RAG System)
- Retrieval-Augmented Generation system for personality analysis
- Combines NLP, embeddings, and LLM reasoning

### Data Analysis Dashboard
- Data processing and visualization using Power BI

### NLP Experiments
- Text classification and sentiment analysis pipelines

---

## GitHub Stats

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=ndjiba-rahal&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ndjiba-rahal&layout=compact&theme=tokyonight" />
</p>

---

## GitHub Streak

<p align="center">
<img src="https://github-readme-streak-stats.herokuapp.com/?user=ndjiba-rahal&theme=tokyonight" />
</p>

---

## GitHub Trophy

<p align="center">
<img src="https://github-profile-trophy.vercel.app/?username=ndjiba-rahal&row=1&column=6" />
</p>

---

## Contact
LinkedIn: https://linkedin.com/in/nadjiba rahal  
Email: rahalnadjiba5@gmail.com

---

## Current Focus
- Building RAG-based AI systems
- Improving Machine Learning and Deep Learning skills
- Developing strong AI engineering portfolio
